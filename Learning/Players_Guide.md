...
- **Clustering** (how territory scatters) and **regional kin** (whether those clusters become *separate* factions) are two different things. **Cluster size** is the largest a single contiguous cluster grows — a faction's territory breaks into clusters of at most this many regions, whether or not it forms kin; `0` means one contiguous nation (no scatter). New clusters are kept spaced apart, so a faction reads as distinct bodies rather than a smear. Cohesive civilisations use one nation; pirates, tribes and rough unions scatter into small clusters.
- **The shattered Empire** is a single, highly-fragmented polity: it scatters into many small clusters (1–3 regions each) spread across the planet, but stays **one faction** — it never forms kin (the toggle is locked off). You can still change its cluster size (down to one contiguous nation) and its relative size; you just can't split it into kin factions.
- **Regional kin sub-factions.** A scattered faction that ends up in several corners of the world can split into related **kin factions**, one per region group, compass-named off the parent (for example *North* and *Southeast* of the base name) and friendly to one another. **Split into regional kin** toggles it per faction (default on for pirates, tribes and rough unions; **off** for the Empire and for spacer-tech factions, whose pod/shuttle mobility keeps them one polity across distance — a kin-off faction still scatters into clusters, it just stays one faction). When kin is on, **Max kin factions** caps how many kin factions the clusters are grouped into (this does *not* set the number of clusters on the map — that is the cluster size). Worldgen caps a world at 40 factions, so beyond that the last factions' kin are truncated.
